...
(function() {
    ...
    var SearchInfoWindow =
        ...
        //alert(this._opts.width +"==="+this._opts.height );
    ...
    SearchInfoWindow.prototype.initialize = function(map) {
        ...
        //alert(this._boxWidth + "+" +this._boxHeight);
        //阻止各种冒泡事件
  ...
  /**
    ...
    baidu.object.extend(SearchInfoWindow.prototype,{
      ...
      _dispatchEvent: function(instance, type, opts) {
         ...
         * @return dom
         */
        /* 源文件_createSearchTemplate: function() {
            if (!this._div) {
                    ...
                    '<div class="BMapLib_bubble_center">',
                        ...
                        '<div class="BMapLib_nav" id="BMapLib_nav' + this.guid + '">',
                            ...
                            '<ul class="BMapLib_nav_tab_content">', //tab内容
                                ...
                                '<li id="BMapLib_transBox' + this.guid + '" style="display:none">',
                                    '<table width="100%" align="center" border=0 cellpadding=0 cellspacing=0>',
                                        '<tr><td width="38" style="padding-left:8px;"><div id="BMapLib_stationText' + this.guid + '">起点</div></td><td width="149"><input id="BMapLib_trans_text' + this.guid + '" class="BMapLib_trans_text" type="text" maxlength="100" autocomplete="off"></td><td width="113" style="padding-left:7px;"><input id="BMapLib_search_bus_btn' + this.guid + '" type="button" value="公交" class="iw_bt" style="margin-right:5px;"><input id="BMapLib_search_drive_btn' + this.guid + '" type="button" class="iw_bt" value="驾车"></td></tr>',
                                    '</table>',
                                '</li>',
                            '</ul>',
                        '</div>',
        ...
        _initService: function () {
            ...
            //renderOptions.autoViewport = true;//3.0
        ...
        /**
         * 公交驾车检索查询
         * 发起检索。 start: 起点，参数可以坐标点或者LocalSearchPoi实例。 end: 终点，
         * 参数可以是坐标点或者LocalSearchPoi实例，3.0版本暂不支持起终点参数为关键字，
         * 开发者可以先用检索接口确认关键字的坐标点
         */
            ...
            if (kw == "") {
                ...
                /*$.getJSON(url, args, function(data) {
                	...
                	if(data != null){
                    	...
                    	console.log(info);
                    	console.log(info.result.location);
                    	tempPoint = info.result.location;
                    	console.log(tempPoint);*/
                    	//go();
                    	/**
                    	 * 在ajax里面使用this关键字需要注意 可以使用第三个变量先接受this  
                    	 * 使用第三变量在ajax里面进行操作
                    	 */
                    	/*var transPoi = mthis._getTransPoi(tempPoint);
                    	console.log(transPoi);
                    	//transitDrive.setPolicy(BMAP_TRANSIT_POLICY_LEAST_TIME);
        ...
        _getTransPoi: function(kw) {
            ...
            if (this._pointType == "startPoint") {
                start = this._point;
            	//start = this._opts.title;
                end   = kw;//从这里出发
            } else {
                start = kw;
                //alert("daozheliqu");
                //alert(kw);
                end   = this._point;//到这里去
                //end   = this._opts.title;
